...
//     delete[] resPtr;
    ...
    std::unique_ptr<int[]> resPtr(itk::cudaRegistrationStart<FixedImageType, MovingImageType, TransformType>(
                        preprocessFixedImage, preprocessMovingImage, transform, fixedImage, movingImage));

    if (resPtr) { // 确保返回的指针不是空指针
        std::copy(resPtr.get(), resPtr.get() + w * h, ptr); // 使用 std::copy 替代手动 for 循环
    }
